...
**版本至上原则**：Apkg 始终以版本号决定哪个包对外服务，而非上传时间。用户无法通过"后发布旧版本"的方式降级已发布的包。

三层防线保证：

**第一层 — 上传时降级守卫** (`DebUploadService.UploadDebToRepositoryAsync` line 126-149)：

若新上传的版本比当前 Primary Bucket 中同 `(Package, Architecture)` 的最高版本**更低**，直接返回 **403 Forbidden**，LocalPackage 不会被创建。错误信息：

> Downgrade blocked: {pkg} {version} ({arch}) is older than the currently published version {latest} in {repo}. Use --allow-downgrade to force the downgrade.

**第二层 — `allowDowngrade` 强制绕过**：

API 端点 (`POST /api/packages/apkg-upload`) 接受 `?allowDowngrade=true` 参数，可强制绕过降级守卫。但注意 — 这只是允许创建 LocalPackage 记录，**不代表最终会被收录**。Web UI 的 Publish 流程不暴露此参数，始终为 `false`。

**第三层 — Sync 去重** (`RepositorySyncJob` line 202-255)：

sync 时，所有 `(Package, Architecture)` 相同的包（包括 LocalPackage 和 Mirror 来的包）按 APT 版本比较规则分组，**只保留版本号最高的那一个**。这意味着：

- 你 force 上传了 v1.0.0，而 Primary Bucket 已有 v2.0.0 → v1.0.0 的 LocalPackage 被创建，但 sync 后 v2.0.0 依然存活，v1.0.0 被删除
- 你发布了 v2.0.0，紧接着又发了一个含 v1.0.0 的 apkg → 同上，v2.0.0 保持对外服务

**结论**：无论上传顺序、是否 force、是否 `--skip-duplicate`，最终 APT 客户端 `apt update` 看到的始终是版本号最高的那个包。

| 场景 | 降级守卫 | Sync 去重 | 最终结果 |
|------|---------|----------|---------|
| 发布 v1.0.0，Primary 已有 v2.0.0 | 403（无 allowDowngrade） | — | v2.0.0 存活 |
| 发布 v1.0.0，force allowDowngrade | 放行 | v2.0.0 > v1.0.0 | v2.0.0 存活 |
| 发布 v3.0.0，Primary 已有 v2.0.0 | 放行（升级） | v3.0.0 > v2.0.0 | v3.0.0 存活 |
| 发布 v2.0.0，Primary 无此包 | 放行（新包） | 唯一版本 | v2.0.0 存活 |

**相关测试**：

- `DebUploadServiceTests.UploadDeb_OlderVersionThanPrimary_WithoutAllowDowngrade_Returns403`
- `DebUploadServiceTests.UploadDeb_OlderVersionThanPrimary_WithAllowDowngrade_AllowsDowngrade`
- `RepositorySyncLocalPackagesTests.SyncJob_LocalPackageOlderThanMirror_WinsAndCausesDowngrade`

**字段分类**：

| 类别 | 字段 | 说明 |
|------|------|------|
| 身份 | `RepositoryId`, `Package`, `Architecture` | 联合索引，决定覆盖范围。**注意：`Component` 不在 LocalPackage 层**——同一仓库内不允许同名同版本同架构的包跨 Component 共存（push 时触发 409 Conflict）。Component 仅在 ApkgPackage 层维护（见 §2 命名约定） |
...
Apkg 服务器完全伪装成标准 Debian/Ubuntu/AnduinOS APT 服务器：
...
- ** (Package, Version, Arch) 槽位冲突**：同仓库内已有同 (Package, Version, Arch) 的已启用包则拒绝，除非使用覆盖模式。（Component 属于 ApkgPackage 层，不在 LocalPackage 上。）
...
**ApkgPackage + ApkgRevision 记录管理**：每次 `apkg push` 在服务端对应一条 `ApkgRevision` 记录，挂载在 `ApkgPackage`（包家族）之下。

- **包家族判定**：按三死属性 (Name, Distro, Component) 查找 `ApkgPackage`。不存在则新建（当前用户为其 Owner），存在且 Owner 不匹配则返回 403。同一用户多次 push 同一 triplet 会在同一 `ApkgPackage` 下累积多个 `ApkgRevision`。
- **前置校验**：Revision 在确认 archive 内所有 `Entry` 引用的 `.deb` 文件都存在之后才创建。manifest 解析失败或文件缺失时直接返回错误，不产生数据库记录。
- **IsPublished 语义**：只有至少有一个 `.deb` 被实际上传且无未解决的错误时才标记为 `Published`。全部被跳过（`--skip-duplicate`）、全部冲突（无 flag 的 409）、或部分成功但存在冲突错误（无 flag）时 Revision 不发布。
- **零上传清理**：如果最终没有任何包被上传（全部跳过或全部冲突），已创建的 `ApkgRevision` 记录会被立即删除（但 `ApkgPackage` 保留）。不会残留"处理过但什么都没做"的垃圾 Revision。
- **部分成功**：部分 repo 上传成功但部分 409 冲突（不带 `--skip-duplicate`）时，Revision 保留（`IsPublished=false`）并返回 409，方便排查哪些 repo 成功、哪些冲突。
- **超时兜底**：极端情况下（如进程在循环中途崩溃），`ApkgTempCleanupJob` 会清理超过 30 分钟仍未发布的 Revision 草稿（见 §12）。

| 场景 | ApkgRevision 结果 |
|------|-------------------|
| 全新包，全部 suite 成功 | 1 条 Published Revision，返回 200 |
| 全部跳过 + `--skip-duplicate` | 0 条 Revision，返回 200 + warnings |
| 全部冲突，不带 flag | 0 条 Revision，返回 409 |
| 部分成功 + `--skip-duplicate` | 1 条 Published Revision（仅含实际上传的包），返回 200 |
| 部分成功，不带 flag | 1 条 Revision（IsPublished=false），返回 409 |
...
| 任务 | 职责 | 触发 |
|------|------|------|
| `ApkgTempCleanupJob` | 清理超过 30 分钟仍未发布的 ApkgRevision 草稿及其临时文件 | 每 10 分钟（startDelay 7 min） |
...
核心实体：ApkgPackage、ApkgRevision、AptMirror、AptRepository、AptBucket、AptPackage、LocalPackage。详细 schema 以 EF Core 迁移代码为准。

- **ApkgPackage**：(Name, Distro, Component) 唯一索引，包家族标识。OwnerUserId 记录首次创建者，后续 push 在此包下累积 ApkgRevision。
- **ApkgRevision**：每次 `apkg push` 一条记录，FK 到 ApkgPackage。记录上传人、时间、文件名、VaultPath 及发布状态（IsPublished/IsListed）。
...
.aosproj 的 `TargetSuites` 允许一个项目为多个 Ubuntu/Debian/AnduinOS 发行版编译。每个 Suite 可能有不同的依赖版本（如 jammy 用 libc6 >= 2.35，noble 用 libc6 >= 2.39）。Condition 语法 `'$(Suite)' == 'jammy'` 让开发者在一个项目文件中表达跨 Suite 差异，而非维护多个分支。
...
| **Apkg*** | 平台级抽象 — 用户可见的概念、上传、SDK、DbContext。这些是 Apkg 在 APT 之上构建的附加值 | `ApkgDbContext`, `ApkgPackage`, `ApkgRevision`, `ApkgPackageManifest` |

规则：如果你在处理 APT 仓库的内部结构（Packages.gz、InRelease、apt-get update），用 `Apt*`。如果你在处理 Apkg 平台功能（用户上传、SDK 工具链、数据库），用 `Apkg*`。

## 17. 已知陷阱与经验教训

### 17.1 arch=all 包的跨 Suite SHA-256 漂移

**现象**：`apt upgrade` 报 `File has unexpected size`，哪怕刚重新发布也无效。

**完整链路**：

```
apkg publish
  ├── noble-addon  CI 编译 foo_1.0.0_all.deb @ mtime=T1  → SHA256=AAA → LocalPackages(repo=noble)
  ├── questing-addon  同一源码 @ mtime=T2              → SHA256=BBB → LocalPackages(repo=questing)
  └── resolute-addon  同一源码 @ mtime=T3              → SHA256=CCC → LocalPackages(repo=resolute)
```

三次上传都通过 slot-conflict 检查（`RepositoryId` 不同，合法）。`RepositorySyncJob` 将三行同步到 `AptPackages`，三行 `Filename` 相同（标准 Debian pool 路径不含 suite），但 SHA256 各异。

apt 从 questing-addon 的 `Packages` index 拿到 SHA256=BBB，但 `GetLocalPoolPath` 无 `ORDER BY` → 随机返回 AAA → 服务了错误的文件 → apt 报哈希不匹配。

**根本原因（两个叠加的 Bug）**：

| Bug | 位置 | 原因 |
|-----|------|------|
| Bug 1 | `DebBuilder.RunCommandAsync` | `dpkg-deb --build` 未设 `SOURCE_DATE_EPOCH=0`，文件 mtime 嵌入 ar/tar header → 同源码不同时间 → 不同 SHA256 |
| Bug 2 | `AptMirrorService.GetLocalPoolPath` | 查询所有 primary bucket 无 `ORDER BY` → `FirstOrDefaultAsync` 随机返回任意一行，可能与当前 Packages index 不一致 |

**修复**：
- Bug 1：在 `RunCommandAsync` 中设 `process.StartInfo.Environment["SOURCE_DATE_EPOCH"] = "0"`
- Bug 2：两个 `FirstOrDefaultAsync` 前加 `OrderByDescending(p => p.BucketId)`

**为什么重新发布没用**：每次 `apkg publish` 针对三个 suite 各构建一次 → 产生三个新的不同 SHA256 → 三条新的冲突行入库，问题立即复现。

**数据结构说明**：`AptPackages` 允许同 `Filename` 存在多行（来自不同 bucket），这是可接受的设计。Bug 1 修好后三行值相同，无害。真正不能接受的是查询时没有确定性排序。

**相关测试**：
- `DebBuilderTests.BuildAsync_RepeatBuildsProduceIdenticalDeb` — 验证同源码两次构建 SHA256 相同
- `AptMirrorServiceTests.GetLocalPoolPath_WhenMultipleSuitesUnderSameDistroHaveDifferentSHA256_ReturnsMostRecentBucket` — 验证返回最新 bucket 的记录

---

### 17.2 同版本号反复发布的幂等性保障

**保障机制**：`DebUploadService.UploadDebToRepositoryAsync` 有两道检查：

1. **SHA256 冲突**（409）：同 repo 内相同文件内容已存在
2. **Slot 冲突**（409）：同 repo 内相同 `(Package, Version, Arch, Component)` 已启用

两道检查确保无论通过 GUI、REST API 还是 apkg CLI 重复上传，都不会写入新的 DB 行或 CAS 文件。

**为什么 arch=all 仍会产生多行**：三个 suite 对应三个不同的 `AptRepository`（不同 `RepositoryId`），slot-conflict 检查是 per-repo 的，因此三次上传合法通过。这是正确行为 — 每个 suite 确实需要独立的 `LocalPackage` 记录。跨 suite 的 SHA256 一致性由 Bug 1 修复（`SOURCE_DATE_EPOCH=0`）来保证。

**相关测试**：
- `DebUploadServiceTests.UploadDeb_SameFileTwice_SecondUploadIsRejectedWithConflict_AndNothingIsWritten`
- `DebUploadServiceTests.UploadDeb_DifferentBinaryForSameSlot_SecondUploadIsRejectedWithSlotConflict`
